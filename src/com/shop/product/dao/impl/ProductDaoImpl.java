...
public class ProductDaoImpl implements ProductDao {
	...
	public List<Product> findHotProduct() {
		...
		// 执行查询(findByCriteria(DetachedCriteria criteria, int firstResult, int
		// maxResults))
	...
	public List<Product> findByPageCid(Integer cid, int begin, int eachPageCount) {
		// SQL语句查询 select p.* from category c,categorysecond cs,product p where c.c_id =
		// cs.c_id and cs.cs_id = p.cs_id and c.c_id = 2
		// HQL语句查询 select p from Category c,CategorySecond cs,Product p where c.c_id =
		// cs.category.c_id and cs.cs_id = p.categorySecond.cs_id and c.c_id = ?
		String hql = "select p from Product p join p.categorySecond cs join cs.category c where c.c_id = ?";
		// 分页查询
		List<Product> list = hibernateTemplate
				.execute(new PageHibernateCallback<Product>(hql, new Object[] { cid }, begin, eachPageCount));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
	...
	public List<Product> findByPageCsid(Integer csid, int begin, int eachPageCount) {
		...
		List<Product> list = hibernateTemplate
				.execute(new PageHibernateCallback<Product>(hql, new Object[] { csid }, begin, eachPageCount));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
	...
	public List<Product> findByPage(int begin, int eachPageCount) {
		String hql = "from Product order by date desc";
		List<Product> list = hibernateTemplate
				.execute(new PageHibernateCallback<Product>(hql, null, begin, eachPageCount));
		if (list != null && list.size() > 0) {
			return list;
		}
		return null;
